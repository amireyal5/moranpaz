    ...
    <main className="min-h-screen bg-background text-right overflow-x-hidden">
      ...
      <section className="relative h-screen w-full flex flex-col items-center justify-center px-6 overflow-hidden bg-white">
        ...
        <div ref={heroReveal} className="relative z-10 text-center reveal flex flex-col items-center max-w-5xl">
           ...
           <h1 className="mb-12">
             <div className="flex flex-wrap justify-center items-center gap-x-4 md:gap-x-8 text-6xl sm:text-8xl md:text-[140px] font-bold leading-none font-['Amatic_SC'] text-foreground">
               <span>להתחבר</span>
           ...
           <h2 className="text-2xl md:text-3xl font-headline italic mb-16 text-accent/70 font-medium">
             פסיכותרפיה הוליסטית וליווי רגשי ליצירת עוגן פנימי
           </h2>
           ...
           <div className="flex flex-col md:flex-row items-center justify-center gap-6">
             <a 
               ...
               className="px-12 py-3.5 bg-primary text-white boutique-label !opacity-100 hover:bg-accent transition-all duration-700 shadow-xl rounded-sm flex items-center gap-4 group"
             >
                תיאום שיחת היכרות
             ...
             <Link 
               href="/practice"
               className="px-10 py-3.5 bg-transparent border border-foreground/15 text-foreground/80 hover:bg-stone-50 transition-all duration-700 text-xs tracking-widest uppercase"
             >
                התהליך הטיפולי
             </Link>
           </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-16 mt-20">
            {[
              ...
              <div key={i} className={cn("text-center md:text-right stagger", `stagger-${i+1}`)}>
                <div className="mb-6 text-primary">{React.cloneElement(point.icon as React.ReactElement, { size: 40, strokeWidth: 0.5 })}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-20">
            {audiences.map((item, i) => (
              <Link 
                key={i} 
                href={item.link}
                className={cn("boutique-card group reveal", `stagger-${i+1}`)}
              >
